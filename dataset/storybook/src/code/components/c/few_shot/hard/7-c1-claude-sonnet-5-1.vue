<template>
  <div class="flex w-2xl flex-col gap-4 p-6">
    <Card
      v-for="notification in notifications"
      :key="notification.name"
      :pt="{
        body: '!p-0 !gap-0',
        content: 'px-4 py-3 border-t border-b border-surface-200',
        footer: 'flex justify-end gap-2 px-4 py-3',
      }"
    >
      <template #header>
        <div class="flex items-center justify-between px-4 py-3">
          <div class="flex items-center gap-3">
            <Avatar :label="notification.initial" shape="circle" />
            <div class="flex flex-col">
              <span class="text-sm font-medium">{{ notification.name }}</span>
              <span class="text-xs text-surface-500">{{ notification.time }}</span>
            </div>
          </div>
          <Tag :value="notification.type" :severity="notification.severity" />
        </div>
      </template>
      <template #content>
        <p class="text-sm text-surface-700">{{ notification.message }}</p>
      </template>
      <template #footer>
        <Button label="Ablehnen" severity="secondary" />
        <Button label="Akzeptieren" severity="primary" />
      </template>
    </Card>
  </div>
</template>

<script setup lang="ts">
  import { ref } from 'vue'
  import Card from 'primevue/card'
  import Avatar from 'primevue/avatar'
  import Tag from 'primevue/tag'
  import Button from 'primevue/button'

  const notifications = ref([
    {
      name: 'Max Mustermann',
      initial: 'M',
      time: 'Vor 5 Minuten',
      type: 'Einladung',
      severity: 'info',
      message: 'Hat dich zum Projekt Website Relaunch eingeladen. Du wirst als Editor hinzugefügt.',
    },
    {
      name: 'Anna Schmidt',
      initial: 'A',
      time: 'Vor 24 Minuten',
      type: 'Review',
      severity: 'warn',
      message: 'Bittet um dein Review für den Pull Request #42 – Neue Navigationsstruktur.',
    },
    {
      name: 'Tom Fischer',
      initial: 'T',
      time: 'Vor 1 Stunde',
      type: 'Zugriff',
      severity: 'danger',
      message: 'Beantragt Admin-Zugriff auf das Dashboard Analytics Q2 2026. Bitte prüfe die Berechtigung.',
    },
  ])
</script>