  ...
  >
    <Tabs value="0">
      ...
      <TabPanels>
        <TabPanel value="0">
          <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-2">
              ...
              <Select v-model="kategorie" :options="kategorieOptions" input-id="kategorie" />
            </div>
            <div class="flex flex-col gap-2">
              <label for="verantwortlicher">Verantwortlicher</label>
              <Select v-model="verantwortlicher" :options="verantwortlicherOptions" input-id="verantwortlicher">
                <template #value="{ value }">
                  <div class="flex items-center gap-2">
                    <Avatar :label="value?.charAt(0)" size="small" shape="circle" />
                    <span>{{ value }}</span>
                  </div>
                </template>
              </Select>
        ...
        <TabPanel value="1">
          <p class="m-0">Zeitplan Inhalt</p>
        </TabPanel>
        <TabPanel value="2">
          <p class="m-0">Einstellungen Inhalt</p>
        </TabPanel>
      </TabPanels>
    </Tabs>
    <template #footer>
...
<script setup lang="ts">
  ...
  const kategorieOptions = ['Entwicklung', 'Design', 'Marketing', 'Vertrieb']
  const verantwortlicher = ref('Max Mustmann')
  const verantwortlicherOptions = ['Max Mustmann', 'Erika Musterfrau']
</script>