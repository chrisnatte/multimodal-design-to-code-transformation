<template>
  <Dialog
    v-model:visible="isProfileEditDialogVisible"
  ...
  >
    ...
    <template #footer>
      <Button label="Abbrechen" severity="secondary" text @click="isProfileEditDialogVisible = false" />
...
<script setup lang="ts">
  ...
  const isProfileEditDialogVisible = ref(true)
  const name = ref('Max Mustmann')
  const role = ref('Administrator')
  const isActive = ref(false)

  const roleOptions = [
    { label: 'Administator', value: 'Administrator' },
