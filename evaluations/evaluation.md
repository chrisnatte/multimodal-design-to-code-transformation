...
**Inwieweit unterscheiden sich regel-, metadaten- und visionsbasierte Transformationsansätze hinsichtlich ihrer Eignung zur automatisierten Generierung von PrimeVue-konformen Benutzeroberflächen von Webanwendungen aus Figma-Mockups?**

## Unterforschungsfragen

1. **Mapping-Präzision**: Welcher Ansatz erzielt die höchste Trefferrate bei der Identifikation und dem framework-spezifischen Mapping von PrimeVue-Komponenten und inwiefern beeinflusst die strukturelle Qualität des Figma-Designs diese Präzision?
...
### UF2: Code-Qualität und Wartbarkeit
...
**Geltungsbereich: 25 (+11 Spezial-) Komponenten.** Der Katalog wird bewusst nur für eine Teilmenge aufgebaut (Formularelemente, Buttons, Panels, Overlays, Menüs sowie 11 zusätzliche Sub-/Spezialkomponenten wie z. B. `AccordionPanel`, `TabPanel`, `Column`; siehe `COMPONENTS`-Liste im Extraktions-Notebook), nicht für alle in `PRIMEVUE_COMPONENTS` geführten Komponenten, die Component-F1 zugrunde liegen. Instanzen von Typen außerhalb des Katalogs tragen nicht zu UF2-Stufe-3 bei.
...
- **Nur ein Teil der PrimeVue-Komponenten abgedeckt** (25 + 11 Spezialkomponenten, siehe Geltungsbereich oben), muss bei der Diskussion der UF2-Ergebnisse angegeben werden, welcher Anteil aller generierten Instanzen überhaupt in die Stufe-3-Auswertung eingehen konnte.
...
| **Code Duplication Rate (CDR)** | Anteil der Zeilen, die Teil eines duplizierten Blocks sind (Mindestlänge z. B. 5 Token), an der Gesamtzeilenzahl der SFC; werden getrennt für `<template>` und `<script>` ermittelt, per Token-basierter Clone-Detection. | Erfasst copy-paste-artige Wiederholung statt Parametrisierung, etwa drei manuell ausgeschriebene, fast identische `<RadioButton>`-Blöcke statt eines `v-for` über ein Options-Array. Eine Änderung müsste sonst an mehreren Stellen statt an einer vorgenommen werden.                                                                                 |
...
| Mean Position-/Size-Similarity | Mittlere, auf die Größe der verglichenen Elemente selbst normalisierte Abweichung von Zentrumsposition bzw. Breite/Höhe gematchter Paare   | Trennt IoU in seine zwei Ursachen auf, sodass unterscheidbar ist, ob ein Element falsch platziert oder falsch dimensioniert wurde                                             |
...
Pro Story wird via injiziertem `EXTRACTION_JS`-Skript der gesamte DOM-Baum unterhalb `document.body` traversiert (nicht nur unterhalb von `#storybook-root`) und für jedes Element erfasst: Tag-Name, `data-pc-name`/`data-pc-section` (PrimeVue-Komponentenkennung, falls vorhanden), Eigentext (nur direkte Text-Kindknoten, nicht der der Nachfahren), Bounding-Box (`getBoundingClientRect()`) sowie 14 `getComputedStyle()`-Properties (Farbe, Typografie, Spacing, Layout), zusätzlich ein `teleported`-Diagnoseflag (`true`, wenn das Element außerhalb von `#storybook-root` liegt). Die Traversierung erfasst dabei bewusst den vollständigen `<body>` statt nur `#storybook-root`, da PrimeVue-Overlay-Komponenten (`Dialog`, `Popover`, `Menu`, `ConfirmDialog`, `Toast`, ...) ihren Inhalt via Vue `Teleport` standardmäßig nach `document.body` rendern, also außerhalb des `#storybook-root`-Teilbaums; eine auf `#storybook-root` beschränkte Traversierung würde solchen Inhalt vollständig übersehen. Zwei Filter verhindern dabei, dass Storybook-eigenes Markup fälschlich als Teil der verglichenen Komponente gezählt wird: Elemente mit den IDs `storybook-docs` bzw. `storybook-highlights-root` (feste Storybook-Container, unabhängig vom Story-Inhalt) werden samt Kindknoten übersprungen, ebenso jeder Teilbaum mit berechnetem `display: none` (deckt u. a. Storybooks eigene, standardmäßig ausgeblendete Preparing-/NoPreview-/ErrorDisplay-Wrapper ab). Ergebnis ist ein JSON-Snapshot pro Story unter `evaluations/visual_fidelity_snapshots/{components|uis}/<approach>/...`. Das `extract_snapshot()`-Retry-Handling fängt dabei nicht nur Timeouts, sondern beliebige Playwright-Fehler ab (z. B. durch Storybooks HMR-Websocket zerstörte Execution-Contexts), da diese bei mehreren hundert bis tausend Stories pro Lauf üblicherweise auftauchen können.
...
mit $w_c = 1{,}0$, $w_t = 0{,}5$, $w_p = 0{,}5$. $\text{comp}(\cdot)$ ist `data-pc-name`, wenn auf mindestens einer Seite vorhanden, sonst der HTML-Tag-Name als Fallback; `textsim` ist die Sørensen-Dice-artige `difflib.SequenceMatcher`-Ratio des Eigentexts (leer/leer $=1{,}0$); `posdist` ist die auf die **Größe der beiden verglichenen Elemente selbst** normalisierte Chebyshev-Distanz der Bounding-Box-Zentren (nicht auf Viewport-Breite/-Höhe, siehe Formel unter „Position-Similarity" weiter unten) und wird hier bewusst **nicht** gedeckelt, sodass ein hinreichend großer Positions-Mismatch (mehr als ca. das 2,4-fache der Elementgröße bei ansonsten identischem Tag und Text) den Match-Cost allein über $\text{MAX\_MATCH\_COST}$ heben und damit einen Match verhindern kann. Eine ursprünglich verwendete, auf Viewport-Breite/-Höhe normalisierte Fassung von `posdist` wurde im Zuge der Auswertung verworfen, da sie auf großformatigen Mockups deutliche, perzeptiv eindeutige Fehlplatzierungen (z. B. ein um mehrere hundert Pixel verschobenes Element) rechnerisch als geringfügig auswies und dadurch sowohl zu nachsichtige Match-Entscheidungen als auch irreführend hohe Position-Similarity-Werte für grob falsch platzierte Elemente erzeugte. Die global kostenminimale 1:1-Zuordnung wird per Ungarischem Algorithmus (`scipy.optimize.linear_sum_assignment`) über die vollständige Kostenmatrix aller generierten × aller Referenz-Elemente bestimmt; Paare mit $\text{cost} > \text{MAX\_MATCH\_COST} = 1{,}2$ werden verworfen, auch wenn sie im Optimum lägen, um bei stark abweichender Elementanzahl keine sinnlosen Zuordnungen zu erzwingen. Nicht gematchte generierte Elemente zählen als **FP**, nicht gematchte Referenzelemente als **FN**, gematchte Paare als **TP**.
...
- **Position-Similarity:** $\max\!\big(0,\ 1 - \max\!\big(|\Delta c_x| / rw,\ |\Delta c_y| / rh\big)\big)$, mit $\Delta c_x, \Delta c_y$ = Differenz der Bounding-Box-Zentren und $rw = (w_g + w_r)/2$, $rh = (h_g + h_r)/2$ = mittlere Breite/Höhe der beiden verglichenen Elemente (statt Viewport-Breite/-Höhe, siehe Begründung beim Match-Cost oben); auf $\geq 0$ gedeckelt, da die elementgrößen-relative Distanz -- anders als die vormals viewport-relative -- größer als $1$ werden kann.
...
| Token-Verbrauch   | Input-/Output-Tokens pro Transformation (nur Ansatz B/C/D, MLLM-basiert) | Grundlage der API-Kosten und unabhängiges Effizienzmaß jenseits der reinen Laufzeit                        |
